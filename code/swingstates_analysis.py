...
def makeaggregated(df, counties, state):
    ...
    for county in counties:
        ...
        for office in offices:
            ...
            margin = dem_total - (rep_total + other_total)
            ...
            new_row = {"State":state,"County":county,"Office":office,"FIPS":fipsmap[state][county],"dem_total":dem_total,"rep_total":rep_total,"other_total":other_total,"margin":margin}

            stats = stats.append(new_row, ignore_index=True)

        print("dem_total %d, rep_total %d, other_total %d" % (totaldem, totalrep, totalother))

        percentmargin = (totalmargin/(totaldem+totalrep+totalother))
